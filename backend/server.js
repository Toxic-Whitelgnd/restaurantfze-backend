...
app.post("/save_current_order", async (req, res) => {
  try {
    ...
    await axios.put(
      `http://localhost:9999/update_table_data/${table_no}`,
      update_data
    );
...
app.put("/update_current_order/:tableno", async (req, res) => {
  ...
  try {
    ...
    await axios.put(
      `http://localhost:9999/update_table_data/${tableno}`,
      update_data
    );
    ...
    await axios.put(
      `http://localhost:9999/update_running_order/${tableno}`,
      send_update_order
    );
...
app.delete("/delete_current_indoor_order/:tableno", async (req, res) => {
  ...
  try {
    ...
    await axios.delete(
      `http://localhost:9999/delete_running_indoor_order/${tableno}`
    );
...
app.delete("/delete_current_outdoor_order/:tableno", async (req, res) => {
  ...
  try {
    ...
    await axios.delete(
      `http://localhost:9999/delete_running_outdoor_order/${tableno}`
    );
...
app.post("/save_customer_details", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Customer Details Saved", order_id: order.id });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).json({ message: "Internal Server Error" });
  }
