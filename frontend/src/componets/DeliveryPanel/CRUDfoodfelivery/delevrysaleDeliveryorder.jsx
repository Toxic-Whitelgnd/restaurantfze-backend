...
const DelevrysaleDeliveryorder = () => {
    ...
    const checkforEmptyItems =  async()=>{
        try {
            ...
            try {
                const response = await axios.delete(`http://localhost:9999/delete_running_deliverysale_order/${emptyitems[0]._id}`);
    ...
    const handleDeleteRunningOrder = async (item,fooditems) => {
        ...
        const res = await axios.put(`http://localhost:9999/update_deliverysale_backend_order/${item.orderno}`,item);
